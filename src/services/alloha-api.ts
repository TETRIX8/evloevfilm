...
export async function fetchAllohaMovieDetails(kinopoiskId: string): Promise<AllohaMovieData | null> {
  if (!kinopoiskId || !kinopoiskId.trim()) {
    console.warn('Alloha API: Invalid kinopoisk ID provided');
    return null;
  }
  ...
  try {
    ...
    if (data.status === 'error' || !data.data) {
      // Only log if it's not the common "not movie" error to reduce console noise
      const errorMessage = data.message || data.error_info || 'Unknown error';
      if (errorMessage !== 'not movie') {
        console.warn('Alloha API:', errorMessage, 'for kinopoisk ID:', kinopoiskId);
      }
      return null;
    }
